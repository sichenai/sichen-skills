...
| [model-connector](./skills/model-connector/SKILL.md) | 自定义大模型自动接入工程师（宿主无关）：双层注册表快路径——常用模型只需说「模型名 + API Key」即可零读文档接入；全部能力/上限经实时 API 探针校验（防虚标/漂移），未知模型自动退回读文档全流程 | 接入自定义模型、多模态能力探测、Token 上限实测、跨 Agent（Claude Code/Cursor/OpenClaw 等）接入自己的模型 |
